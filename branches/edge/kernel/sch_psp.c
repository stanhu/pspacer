...
static inline clock_delta recalc_gapsize(unsigned long parent_rate,
				unsigned long rate, clock_delta base,
				unsigned int cut)
{
	u64 ipg=parent_rate;
	...
	ipg = mul_div(ipg,base-cut,rate);
	ipg -= min_t(u64,ipg,base);
	return ipg;
}
 ...
 */
static clock_delta update_clocks(struct sk_buff *skb, struct Qdisc *sch,
			    struct psp_class *cl, int len, unsigned long *hw_gap)
{
	...
	if (cl->parent) {
	    ...
	    q->clock0 =
#endif
	    q->clock += len + gapsize1;
	    p_rate=q->max_rate;
#if 1
	    /* IMHO, but vs. original "+ FCS" */
	    *hw_gap += len2;
	    /* once I calculate to exclude IP header, but forget logic */
	    // *hw_gap += 32;
#else
	...
	case TC_PSP_MODE_DYNAMIC:
		...
		gapsize = recalc_gapsize(p_rate, cl->rate, gapsize1+len, *hw_gap);
	...
	*hw_gap += cl->hw_gap;
	...
	if (!(cl->state & FLAG_DMARK)) {
		...
		/* reset class clock */
		cl->state &= ~FLAG_DMARK;
		cl->clock = q->clock + gapsize - *hw_gap;
	}
...
static struct psp_class *lookup_next_class(struct Qdisc *sch, clock_delta *gapsize)
{
	...
	list_for_each_entry(cl, &q->pacing_list, plist) {
	    if (found == NULL && cl->level==0) {
		clock = max_clock(cl);
		if(clock > q->clock) {
		    if (nearest <= (diff = clock - q->clock))
			continue;
		    nearest = diff;
		    continue;
		}
...
static inline int retrans_check(struct psp_class *cl, struct sk_buff* skb)
{
...
#define TH ((struct tcphdr *)th)
	...
	x=jhash_3words(iph->saddr,iph->daddr,*(u32 *)th,0);
    } else
#ifdef CONFIG_IPV6
	...
	x=jhash(&iph->saddr,32,0);
	th=(unsigned char *)&iph[1];
	x=jhash2((u32 *)th,1,x);
    } else
#endif
	    return -1;
    tcp_size=skb->len-(th-skb->data)-(TH->doff<<2);
...
next_pkt:
    ...
    // h->end+=seq+(TH->syn?1:tcp_size)+TH->fin; /* rfc793 */
    h->end+=seq+tcp_size+TH->syn+TH->fin;
continue_connection:
#ifdef TTL
    h->next=NULL;
...
#define psp_destroy_filters(fl) tcf_destroy_chain(*(fl))
#else
#define psp_destroy_filters tcf_destroy_chain
#endif
