...
#define nla_parse(tb,max,head,len,policy0) rtattr_parse(tb,max,head,len)
#define nla_parse_nested(tb,max,nla,policy0) rtattr_parse_nested(tb,max,nla)
...
#define nla_nest_end(skb,nla) nla->rta_len = skb_tail_pointer(skb) - b
#define nla_nest_start(skb,opt) (struct nlattr *)b; NLA_PUT((skb), (opt), 0, NULL)
#define nla_nest_cancel(skb, nla) skb_trim((skb), b - (skb)->data)
#define _OPT(x) (x)-1
#else
#define _OPT(x) (x)
#endif
